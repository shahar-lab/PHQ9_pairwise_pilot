...
plot_df <- data.frame(
  true_val      = u_recovery$true_value,
  recovered_val = u_recovery$recovered_value
)
plot_df <- plot_df[complete.cases(plot_df), ]
stopifnot(nrow(plot_df) == nrow(u_recovery))
...
p_u <- ggplot(plot_df, aes(x = true_val, y = recovered_val)) +
  ...
  labs(x = "True utility (u)", y = "Recovered utility (u)")

ggsave(file.path(output_dir, "u_recovery.pdf"), plot = p_u, width = 10, height = 8, bg = "white")
ggsave(file.path(output_dir, "u_recovery.png"), plot = p_u, width = 10, height = 8, dpi = 300, bg = "white")
