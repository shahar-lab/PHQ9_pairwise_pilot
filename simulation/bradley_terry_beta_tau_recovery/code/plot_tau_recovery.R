...
plot_df <- data.frame(
  true_val      = tau_recovery$true_value,
  recovered_val = tau_recovery$recovered_value
)
plot_df <- plot_df[complete.cases(plot_df), ]
stopifnot(nrow(plot_df) == nrow(tau_recovery))
...
p_tau <- ggplot(plot_df, aes(x = true_val, y = recovered_val)) +
  ...
  labs(x = "True tau", y = "Recovered tau")

ggsave(file.path(output_dir, "tau_recovery.pdf"), plot = p_tau, width = 10, height = 8, bg = "white")
ggsave(file.path(output_dir, "tau_recovery.png"), plot = p_tau, width = 10, height = 8, dpi = 300, bg = "white")
